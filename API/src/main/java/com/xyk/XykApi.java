...
public class XykApi {
     ...
     * @param roundingMode 舍入方式,示例:RoundingMode.UP表示向上取整,传入null时,以四舍五入计算
...
//        return sb.toString().equals(str);
    ...
    public static <T> T BeanUtils_mapToObject(Map<Object, Object> map, Class<T> beanClass) throws Exception {
        ...
        for (Field field : fields) {
            ...
            if (map.containsKey(field.getName())) {
                field.set(bean, XykApi.BeanUtils_objectValueOf(value, field.getType()));
            }
        }
        return bean;
    }
    ...
    /**
     * 获取本机的ip/host,windows和linux通用,运行的端口可以通过本类的属性port获取
     * 方法id [07]001
     */
     ...
     * @param out              压缩文件输出流(禁止把压缩文件输出到压缩文件路径下,否则会死循环,存储空间占满)
     * @param keepDirStructure 是否保留原来的目录结构,
     *                         true:保留目录结构;
     *                         false:所有文件跑到压缩包根目录下(注意：不保留目录结构可能会出现同名文件,会压缩失败)
     * @throws RuntimeException 压缩失败会抛出运行时异常
     */
    public static void FileUtils_dirToZip(String srcDir, OutputStream out, boolean keepDirStructure) throws RuntimeException {
        ...
        try {
            ...
            throw new RuntimeException("压缩文件时出现异常!", e);
        } finally {
            if (zos != null) {
                try {
                    zos.close();
    ...
    /**
     * 读取本地.txt文件内容,返回一个很长的字符串(保留原文档格式)
     * 方法id [08]002
     *
    ...
    /**
     * 向txt文本中写入文本(文件不存在则创建并写入:可选择重写还是追加)
     * 方法id [08]003
     *
     * @param FilePathName txt文本文件的全路径名 示例:C:\Users\12561\Desktop\Demo.txt
    ...
    public static void FileUtils_WriteStrToFile_Txt(String FilePathName, String content, Boolean append, Boolean changeLine) {
        ...
        try {
            ...
            if (!exists) {
                file.createNewFile();
                System.out.println("目标文件不存在,已创建目标文件:[" + FilePathName + "]!");
            }
            fos = new FileOutputStream(file, append); // 是否追加写入
            bw = new BufferedWriter(new OutputStreamWriter(fos));
            if (exists && append && changeLine) { // 只有文件已经存在,确定追加,确定换行,三个条件同时满足才执行换行后写入
                bw.newLine();
            }
            bw.write(content);
